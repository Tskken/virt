...
fn main() {
    ...
    let vertex_buffer = {
        ...
        let pos1 = Point::new(-0.5, -0.25);
        let pos2 = Point::new(0.0, 0.5);
        let pos3 = Point::new(0.25, -0.1);

        println!("pos1: {:?}, pos2: {:?}, pos3: {:?}", pos1, pos2, pos3);
        
        let dimensions: [f64; 2] = surface.window().inner_size().into();

        let sizev = Point::new(dimensions[0], dimensions[1]);

        let newpos1 = pos1.project(sizev);
        let newpos2 = pos2.project(sizev);
        let newpos3 = pos3.project(sizev);

        println!("newpos1: {:?}, newpos2: {:?}, newpos3: {:?}", newpos1, newpos2, newpos3);

        let p1: [f32; 2] = {
            let p = newpos1.unproject(sizev);

            [p.x as f32, p.y as f32]
        };

        let p2: [f32; 2] = {
            let p = newpos2.unproject(sizev);

            [p.x as f32, p.y as f32]
        };

        let p3: [f32; 2] = {
            let p = newpos3.unproject(sizev);
